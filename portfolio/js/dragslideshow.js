...
;( function( window ) {
	...
	'use strict';
	// let this area react to scroll events
	var slideShow = document.getElementById( 'slideshow' ),	
	page = slideShow.querySelector( '.pages' ),
	...
	DragSlideshow.prototype._initEvents = function() {
		...
		this.slides.forEach( function( slide ) {
			...
			slide.addEventListener( 'click', function() {
				...
				// Removed logic to jump to slide view in the menu vs. just launching on click
				/*
			...
			// reveal content on on scroll
			if (slideShow.addEventListener) {
				// IE9, Chrome, Safari, Opera
				slideShow.addEventListener("mousewheel", function(e) {
					...
					if( self.isContent ) {
						if( delta === 1 ) {
							if( self.isContent ) {
								// Need to get the scrollTop value of the current page somehow
								if( page.scrollTop <= 0) { self._toggleContent( currentSlide ); page.scrollTop = 0}
							}
						} 
					} else {
						//console.log("launch screen active");
						if ( delta === -1 ) {
				...
				// Firefox
				slideShow.addEventListener("DOMMouseScroll", function(e) { 
					var currentSlide = self.slides[ self.current ];
				...
				slideShow.attachEvent("onmousewheel", function(e) {
					var currentSlide = self.slides[ self.current ];
	...
	DragSlideshow.prototype.onHandleTouchStart = function(e) {
			Cursor.refresh(e);
